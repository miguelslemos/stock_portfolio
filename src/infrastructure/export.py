    ...
    def _convert_positions_to_records(self, positions: List[PortfolioPosition]) -> List[Dict[str, Any]]:
        ...
        for position in positions:
            ...
            record = {
                ...
                'Total Cost USD': float(position.total_cost_usd.amount),
                'Average Price USD': float(position.average_price_usd.amount),
                'Total Cost BRL': float(position.total_cost_brl.amount),
                'Average Price BRL': float(position.average_price_brl.amount),
                'Gross Profit BRL': float(position.gross_profit_brl.amount)
            }
